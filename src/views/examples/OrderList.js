                            ...
                            body: JSON.stringify(
                                {
                                    ...
                                    "notification": {
                                        "body": "Your order '" + orderInfo.order_id + "' has been approved",
                                        "title": "Deltrix Order",
        ...
        <Table className="align-items-center table-flush" responsive>
            {/* <Button color="" onClick={() => {
                    ...
                    fetch('https://fcm.googleapis.com/fcm/send', {
                        ...
                        body: JSON.stringify(
                            {
                                "registration_ids": ["fZRwWnLxQ1Gxrjg988D4eE:APA91bFmLYQ12v-QNLBTphW0J1vdMp2enz-U8pLbyoY0W79YZnVnz_v2-w9t8PS7LtHXUYGMhsOoK76MgaBVbacEB3CrXNlPZYKXqaxEOE06yR_XfTT_EZZg88rVvwDYzumsPfL8hQYW"],
                                "notification": {
                                    "body": "great match!",
                                    "title": "Portugal vs. Denmark",
                                    "content_available": true,
            ...
            <tbody style={{ textAlign: "center" }}>
                {orderList.map((v, x) => (
                    <tr key={x} >
                        ...
                        <th>
                            <Button color="info" onClick={() => ViewDetail(v)}>{"View more"}</Button>
                        </th>
                    </tr>
                ))}
            </tbody>
            <Modal style={{ width: "90%" }} size={"larg"} isOpen={modal} className="modal-dialog-order-list" toggle={toggle} >
                <ModalBody>
                    <OrderModal orderDetail={orderData} />
                </ModalBody>
                <ModalFooter>
                    <Button color="" onClick={() => toggle()}>Close</Button>
                    {orderData.status == "approved" ?
                        <Button color="red" onClick={() => handlePrint()}>Print</Button>
                        : null}
                        ...
                        <NavItem>
                            <NavLink to={"#"} tag={NavLinkRRD} activeClassName="active">
                                {orderData.status == "panding" ?
                                    <Button color="info"
                                        onClick={() => updateStatus(orderData._id, orderData, "approved")}>Approve</Button> :
                                    orderData.status == "approved" ?
                                        <Button color="info" onClick={() => updateStatus(orderData._id, "delivered")}>Delivered</Button> : null}
                            </NavLink>
                            {/* <NavLink to={"#"} tag={NavLinkRRD} activeClassName="active">
                            </NavLink> */}
                        </NavItem>
        ...
        get(url)
            .then(({ data }) => {
                if (data && data.products.length > 0) {
                    console.log(data)
                    this.setState({ orderList: orderList.concat(data.products), }, () => {
                    })
                }
            }).catch(err => {
                console.log(err)
                                        ...
                                        <Nav tabs>
                                            <NavItem>
                                                <NavLink
                                                    ...
                                                    onClick={() => { this.toggleTab('1', "panding"); }}
                                                >
                                                    Pending
                                               </NavLink>
                                            </NavItem>
                                            <NavItem>
                                                <NavLink
                                                    ...
                                                    onClick={() => { this.toggleTab('2', "approved"); }}
                                                >
                                                    Approved
                                        </NavLink>
                                            </NavItem>
                                            <NavItem>
                                                <NavLink
                                                    ...
                                                    onClick={() => { this.toggleTab('3', "delivered"); }}>
                                                    Delivered
                                                 </NavLink>
                                            </NavItem>
                                        </Nav>
